...
export const createTRPCContext = async (opts: { headers: Headers }) => {
  // For NextAuth v5 with JWT sessions, use getToken to get the JWT payload
  let session = null;

  try {
    const token = await getToken({
      req: { headers: Object.fromEntries(opts.headers.entries()) } as { headers: Record<string, string> },
